...
set_mixer_control! lpf: 140, lpf_slide: 2
...
with_fx :reverb, room: 0.8 do
  live_loop :ground, sync: :metro do
    4.times do
      ...
      with_fx :distortion, amp: 0.6, distort: 0.3 do
        with_fx :eq, high: 1.5, high_shelf: 0 do
          if spread(14, 16).rotate(num2).tick(:fm1)
            synth :fm,
              note: notes4 + choose([12,24,36]),
              depth: 0,
              devisor: 2,
        ...
        if spread(16, 32).rotate(num).tick(:tb1)
          synth :tb303,
            note: notes-12,
            res: (range 0, 0.9, 0.05).mirror.look(:tb1),
  ...
  live_loop :bd, sync: :metro do
    ...
    with_fx :lpf, cutoff: 80, cutoff_slide: 16 do
      with_fx :eq, amp: 5, low_shelf: -1, low: -0.8, mid: -0.6, high: 0.5 do
        ...
        sample :bd_haus, amp: 3, sustain: 0.25, rate: 0.7, cutoff: 90 if(spread 1,4).tick
      end
    end
    #stop
    sleep 0.75
...
with_fx :reverb, room: 0.1 do
  live_loop :pc, sync: :metro do
    ...
    with_fx :eq, amp: 3, low: -2, high: -0.2 do
      #sample :drum_cymbal_soft, amp: 1, sustain: 0.1, release: 0.2 if(spread 1, 6).tick
      #sample :drum_cymbal_closed, amp: 0.4, sustain: 0.1, release: 0.2 if(spread 1, 4).tick
    end

    with_fx :eq, amp: 2, low: -1, high: -0.1 do
      #sample :drum_cymbal_soft, amp: 1, sustain: 0.1, release: 0.1 if(spread 1, 6).tick
      #stop
    end
